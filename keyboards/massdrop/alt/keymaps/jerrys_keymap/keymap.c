...
LM(1, MOD_LSFT), KC_Z,    KC_X,    KC_C,    KC_V,    KC_B,    KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT,          KC_UP,   KC_PGDN, \
        KC_LCTL, KC_LGUI, LM(1, MOD_LALT),                    KC_SPC,                             KC_RALT, MO(2),   KC_LEFT, KC_DOWN, KC_RGHT  \
    ),
    ...
    [2] = LAYOUT(
        ...
        L_T_PTD, L_PTP,   L_BRD,   L_PTN,   L_EDG_D, _______, _______, _______, _______, _______, _______, _______,                                _______, KC_MUTE            , \
        _______, L_T_MD,  L_T_ONF, _______, L_EDG_M, MD_BOOT, TG_NKRO, _______, _______, _______, _______, _______,                      KC_AUDIO_VOL_UP  , KC_MEDIA_PLAY_PAUSE, \
        _______, _______, _______,                            DBG_FAC,                            _______, _______, KC_MEDIA_PREV_TRACK, KC_AUDIO_VOL_DOWN, KC_MEDIA_NEXT_TRACK  \
    ),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case L_T_MD:
            if (record->event.pressed) {
                // Changing up the order of the led mode rotation
                if  (led_lighting_mode == LED_MODE_KEYS_ONLY)
                    led_lighting_mode = LED_MODE_NORMAL;
                else if (led_lighting_mode == LED_MODE_NORMAL)
                    led_lighting_mode = LED_MODE_NON_KEYS_ONLY;
                else
                    led_lighting_mode++;
                if (led_lighting_mode > LED_MODE_MAX_INDEX) led_lighting_mode = LED_MODE_KEYS_ONLY;
            }
            return false;
        case L_T_ONF:
            if (record->event.pressed) {
